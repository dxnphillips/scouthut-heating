...
| A29b Cold booking pierces the lockout | `_cold_booking_bypass(zone)` — the `seasonal_lockout` rung in `_desired_zone` returns `ice` **only** when no cold booking bypasses it. A bypass needs: the **Cold bookings heat despite lockout** switch on (default), an active booking or pre-heat window (`_cal_active`, so the pre-heat is covered — a cold booked morning is warm from the first minute), and the room's **coldest heater probe** below the booking target (`_booking_target` — comfort, or eco-low for an ECO-keyword event). Self-calibrating: a warm-fabric summer booking already at target does not bypass, so this is a cold-snap escape hatch, not a season-long defeat of the lockout. Release hysteresis (`COLD_BOOKING_RELEASE_BAND` = 0.5 °C, keyed off the applied preset) holds the pierce until the room sits half a degree above target, so it cannot flap around the setpoint. An **unreadable** room does not bypass (summer fail-safe: stay locked; a manual Boost still pierces). The pierced booking then falls through to the normal booking rungs, so it drives the reverse/destrat fan regime like any heated hall (see the *Heating forces reverse* row). The shared zone follows via `_desired_shared` (`_cold_booking_bypass` on either zone → `eco`). Audit `reason` is tagged `lockout_booking` / `lockout_preheat` / `lockout_booking_eco` / `lockout_booking_quiet` so a lockout-piercing session is greppable. |
...
| --- | --- |
| Winter destratification (up air) | `_fan_target` + `fan_decision`: ceiling-floor ΔT above `fan_dt_on` **and** the heat is worth moving — `_heat_demand()` true (any Rointe *Effective Power* over `heat_demand_watts`, across hall/office/shared) **or** the floor is below `fan_recirc_max_floor_temp`. The recirculation term decouples the fans from the heater's on/off cycle, so residual heat is harvested after a heater cuts out — but with `winter_fans_need_occupancy` (default on) that no-demand recirc path also requires hall occupancy (`_cooling_occupied()`), so an empty, unheated hut does not run the fans on warm-fabric ambient stratification alone (the 2026-07-13 export showed that as ~150 W of continuous cost with, per the cool-off data, no measurable retention gain). Active `_heat_demand()` still runs the fans regardless of occupancy (the savings case, including pre-heat). Turn the switch off for the legacy run-on-stratification-alone behaviour. Hysteresis via `fan_dt_off`, `fan_min_run_minutes`, `fan_min_off_minutes`. Direction reverse. |
