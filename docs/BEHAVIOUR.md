...
| A29b Cold booking pierces the lockout | `_cold_booking_bypass(zone)` — the `seasonal_lockout` rung in `_desired_zone` returns `ice` **only** when no cold booking bypasses it. A bypass needs (this is always-on behaviour — freezing a genuinely cold booked room is never wanted): an active booking or pre-heat window (`_cal_active`, so the pre-heat is covered — a cold booked morning is warm from the first minute), and the room's **coldest heater probe** (freshness-gated — a frozen Rointe reading is rejected) below the booking target (`_booking_target` — comfort, or eco-low for an ECO-keyword event). Self-calibrating: a warm-fabric summer booking already at target does not bypass, so this is a cold-snap escape hatch, not a season-long defeat of the lockout. Release hysteresis (`COLD_BOOKING_RELEASE_BAND` = 0.5 °C, keyed off the applied preset) holds the pierce until the room sits half a degree above target, so it cannot flap around the setpoint. An **unreadable** room does not bypass (summer fail-safe: stay locked; a manual Boost still pierces). The pierced booking then falls through to the normal booking rungs, so it drives the reverse/destrat fan regime like any heated hall (see the *Heating forces reverse* row). The shared zone follows via `_desired_shared` (`_cold_booking_bypass` on either zone → `eco`). Audit `reason` is tagged `lockout_booking` / `lockout_preheat` / `lockout_booking_eco` / `lockout_booking_quiet` so a lockout-piercing session is greppable. |
| A29c State-based summer setback | `_summer_setback_wants_heat()` — with the **Summer setback (state-based) mode** switch on (default **off**), the `seasonal_lockout` rung stops being a hard block for the **hall**: an *occupied* hall (`_cal_active` OR recent hall motion OR the occupied override) whose **averaged floor** is below `hall_summer_comfort_temp` (default 17.5) is warmed to that floor via the **eco** preset (`reason` = `summer_setback`) instead of frozen to `ice`. Answers audit finding F2 — in summer, occupancy alone previously earned no heat; only a booking could. The eco setpoint carries the setback number, not the ordinary eco value: `_hall_eco_target(eco_low)` routes to `hall_summer_comfort_temp` whenever the reason is `summer_setback` (delivered via eco because the Rointe comfort floor is 19 °C and the setback wants ~17.5; every other eco path keeps the ordinary number). Reads the *averaged* floor, not the *coldest* probe the cold-booking pierce uses — a low-priority comfort floor must not over-fire on one cool end. A warm hall (≥ floor) or an empty hall still lands on `ice` (summer cooling fans keep it); an **unreadable** room does not heat (summer fail-safe: err off). Release hysteresis reuses `COLD_BOOKING_RELEASE_BAND` (0.5 °C, keyed off the applied preset). Because the hall lands on a heating preset the fans run the reverse/destrat regime (see *Heating forces reverse*). A genuinely cold **booking** still wins full **comfort** through the cold-booking pierce (checked first). Hall/`ZONE_A` only; office/shared unaffected. |
